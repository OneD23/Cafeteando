    ...
    <TouchableOpacity style={styles.primaryBtn} onPress={handleCreateUser} disabled={isSaving}><Text style={styles.primaryText}>{isSaving?'Guardando...':'Crear Usuario'}</Text></TouchableOpacity>
      ...
      <View style={styles.invoiceList}>
        <View style={styles.invoiceHeader}>
          <Text style={[styles.invoiceHeaderText, styles.invoiceColId]}>Factura</Text>
          <Text style={[styles.invoiceHeaderText, styles.invoiceColDate]}>Fecha</Text>
          <Text style={[styles.invoiceHeaderText, styles.invoiceColTotal]}>Total</Text>
        </View>
        {selectedClientInvoices.map((inv: any) => (
          <TouchableOpacity key={`${inv.saleId}-${inv.date}`} onPress={() => setSelectedInvoice(inv)} style={styles.invoiceRow}>
            <Text style={[styles.invoiceItem, styles.invoiceColId]}>#{inv.saleId}</Text>
            <Text style={[styles.invoiceItem, styles.invoiceColDate]}>{new Date(inv.date).toLocaleString()}</Text>
            <Text style={[styles.invoiceItem, styles.invoiceColTotal]}>${Number(inv.total || 0).toFixed(2)}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </>}
